...
export class RestApiNotificationsTab {
  ...
  constructor(modal: Locator) {
    this.notificationGroupsDualPaneSelector = new DualPaneSelector(modal.locator('.label:has-text("Notification Groups") + .data'));
    this.notificationUsersDualPaneSelector = new DualPaneSelector(modal.locator('.label:has-text("Notification Users") + .data'));
  }
